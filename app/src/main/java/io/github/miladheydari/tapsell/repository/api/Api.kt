...
class Api(private val api: RetrofitInterface, private val schedulerProvider: SchedulerProvider) {
    ...
    fun sendData(@Body data: NoteModel): Single<Response> = api.sendData(data).observeOn(schedulerProvider.mainThread()).subscribeOn(schedulerProvider.backgroundThread())
